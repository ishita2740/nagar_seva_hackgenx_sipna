...
function ensureUsersRoleSupportsContractor(database: DatabaseSync) {
  const row = database
...
function ensureGrievancesWorkflowSchema(database: DatabaseSync) {
  const row = database
...
function normalizeGrievanceWorkflowData(database: DatabaseSync) {
  database.prepare(
    `
      UPDATE grievances
      SET status = CASE
